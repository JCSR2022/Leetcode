...
#     def treeQueries(self, root: Optional[TreeNode], queries: List[int]) -> List[int]:
        ...
        #brute force, find tree height whithout node in query
        
#         def dfs_height(node,exclude,height = 0):
#             if node and node.val != exclude:    
#                 height +=1  
#                 ans1 = dfs_height(node.left,exclude,height)   
#                 ans2 =dfs_height(node.right,exclude,height) 
#                 return max(height,ans1,ans2)
#             return 0
        
#         heights = []
#         for exclude in queries:
#             heights.append(dfs_height(root,exclude)-1)
#         return heights